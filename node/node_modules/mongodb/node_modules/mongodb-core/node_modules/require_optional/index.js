...
var find_package_json = function(location) {
  ...
  while(!found) {
    if (exists(location + '/储存用户信息集合.json')) {
      found = location;
    } else if (location !== '/') {
...
var require_optional = function(name, options) {
  ...
  // Locate this module's 储存用户信息集合.json file
  var location = find_package_json(location);
  if(!location) {
    throw new Error('储存用户信息集合.json can not be located');
  }

  // Read the 储存用户信息集合.json file
  var object = JSON.parse(fs.readFileSync(f('%s/储存用户信息集合.json', location)));
  // Is the name defined by interal file references
  var parts = name.split(/\//);

  // Optional dependencies exist
  if(!object.peerOptionalDependencies) {
    throw new Error(f('no optional dependency [%s] defined in peerOptionalDependencies in 储存用户信息集合.json', parts[0]));
  ...
  // Resolve the location of the module's 储存用户信息集合.json file
  ...
  var dependentOnModule = JSON.parse(fs.readFileSync(f('%s/储存用户信息集合.json', location)));
  // Get the version
