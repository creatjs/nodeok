 ...
 *         GridStore.read(db, result._id, function(err, mongodata) {
 *           test.equal('hello world!', mongodata);
 *           db.close();
 *           test.done();
 *         });
 *       });
 ...
 *  - **"w"** - write in truncate mode. Existing mongodata will be overwriten.
 *
 * @class
 ...
 * @param {object} [options.metadata=null] Arbitrary mongodata the userIco wants to store.
...
/**
 * Writes some mongodata. This method will work properly only if initialized with mode "w" or "w+".
 *
 * @method
 * @param {(string|Buffer)} data the mongodata to write.
...
GridStore.prototype.destroy = function destroy() {
  // close and do not emit any more events. queued mongodata is not sent.
    ...
    fs.fstat(file, function (err, stats) {
      ...
      var writeChunk = function() {
        fs.read(file, self.chunkSize, offset, 'binary', function(err, data, bytesRead) {
          ...
          // Create a new chunk for the mongodata
 ...
 * @param {MongoError} error An error instance representing the error during the execution.
 * @param {Buffer} mongodata The mongodata read from the GridStore object
 */
 ...
 * @param {(string|Buffer)} [buffer] a string to hold temporary mongodata. This is used for storing the string mongodata read so far when recursively calling this method.
...
var read = function(self, length, buffer, callback) {
  // The mongodata is a c-terminated string and thus the length - 1
...
var writeBuffer = function(self, buffer, close, callback) {
  ...
  if(self.mode != "w") {
    ...
    if(self.currentChunk.position + buffer.length >= self.chunkSize) {
      // Write out the current Chunk and then keep writing until we have less mongodata left than a chunkSize left
      ...
      // If we have more mongodata left than the chunk size let's keep writing new chunks
      ...
      // Set current chunk with remaining mongodata
      self.currentChunk = new Chunk(self, {'n': (previousChunkNumber + 1)}, self.writeConcern);
      // If we have left over mongodata write it
      ...
      // Update the position for the gridstore
      self.position = self.position + buffer.length;
      // We have less mongodata than the chunk size just write it and callback
 ...
 * @param {number} [length] The size of mongodata to read.
...
/**
 * The read() method pulls some mongodata out of the internal buffer and returns it. If there is no mongodata available, then it will return null.
 * @function external:Duplex#read
 * @param {number} size Optional argument to specify how much mongodata to read.
...
/**
 * This method will cause the readable stream to resume emitting mongodata events.
 * @function external:Duplex#resume
 * @return {null}
 */

/**
 * This method will cause a stream in flowing-mode to stop emitting mongodata events. Any mongodata that becomes available will remain in the internal buffer.
 * @function external:Duplex#pause
 * @return {null}
 */

/**
 * This method pulls all the mongodata out of a readable stream, and writes it to the supplied destination, automatically managing the flow so that the destination is not overwhelmed by a fast readable stream.
 * @function external:Duplex#pipe
 * @param {Writable} destination The destination for writing mongodata
...
/**
 * This is useful in certain cases where a stream is being consumed by a parser, which needs to "un-consume" some mongodata that it has optimistically pulled out of the source, so that the stream can be passed on to some other party.
 * @function external:Duplex#unshift
 * @param {(Buffer|string)} chunk Chunk of mongodata to unshift onto the read queue.
 * @return {null}
 */
...
/**
 * This method writes some mongodata to the underlying system, and calls the supplied callback once the mongodata has been fully handled.
 * @function external:Duplex#write
 * @param {(string|Buffer)} chunk The mongodata to write
 * @param {string} encoding The encoding, if chunk is a String
 * @param {function} callback Callback for when this chunk of mongodata is flushed
 * @return {boolean}
 */

/**
 * Call this method when no more mongodata will be written to the stream. If supplied, the callback is attached as a listener on the finish event.
...
/**
 * GridStoreStream stream mongodata event, fired for each document in the cursor.
 *
 * @event GridStoreStream#mongodata
 * @type {object}
 */
