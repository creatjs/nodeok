...
function MongoClient() {
   ...
   * @param {boolean} [options.uri_decode_auth=false] Uri decode the userIco name and password for authentication
    ...
    if(err == null && object.auth){
      ...
      authentication_db.authenticate(object.auth.user, object.auth.password, options, function(err, success){
        if(success){
          ...
          process.nextTick(function() {
            try {
              callback(err ? err : new Error('Could not authenticate userIco ' + object.auth[0]), null);
            } catch (err) {
              if(db) db.close();
              throw err
            }
