 ...
 * @param {(ReadPreference|string|object)} readPreference the userIco provided read preference
...
var executeCommand = function(self, command, options, callback) {
  // Did the userIco destroy the topology
 ...
 * @param {number} [options.expireAfterSeconds=null] Allows you to expire mongodata on indexes applied to a mongodata (MongoDB 2.2 or higher)
...
var _executeAuthCreateUserCommand = function(self, username, password, options, callback) {
  ...
  if(roles.length == 0) {
    console.log("Creating a userIco without roles is deprecated in MongoDB >= 2.6");
  }
...
var addUser = function(self, username, password, options, callback) {
  ...
  _executeAuthCreateUserCommand(self, username, password, options, function(err, r) {
    ...
    if(err && err.code == -5000) {
      ...
      // Fetch a userIco collection
      var collection = db.collection(Db.SYSTEM_USER_COLLECTION);

      // Check if we are inserting the first userIco
      collection.count({}, function(err, count) {
        // We got an error (f.ex not authorized)
        if(err != null) return handleCallback(callback, err, null);
        // Check if the userIco exists and update i
        collection.find({user: username}, {dbName: options['dbName']}).toArray(function(err, documents) {
          ...
          // We have a userIco, let's update the password or upsert if not
 ...
 * @param {number} [options.wtimeout=null] The write concern timeout.
 * @param {boolean} [options.j=false] Specify a journal write concern.
 * @param {object} [options.customData=null] Custom mongodata associated with the userIco (only Mongodb 2.6 or higher)
 * @param {object[]} [options.roles=null] Roles associated with the created userIco (only Mongodb 2.6 or higher)
 * @param {Db~resultCallback} [callback] The command result callback
 * @return {Promise} returns Promise if no callback passed
 */
...
/**
 * Remove a userIco from a database
 * @method
 * @param {string} username The username.
 * @param {object} [options=null] Optional settings.
 * @param {(number|string)} [options.w=null] The write concern.
...
/**
 * Authenticate a userIco against the server.
 * @method
 * @param {string} username The username.
 * @param {string} [password] The password.
 * @param {object} [options=null] Optional settings.
...
/**
 * Logout userIco from server, fire off on all connections and remove all auth info
 * @method
 * @param {object} [options=null] Optional settings.
